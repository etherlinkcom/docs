...
The upgrade also includes foundational work to the XTZ precompiled contract that is a step toward allowing faster withdrawals from Etherlink to Tezos layer 1.
...
This event is now emitted by the Tezos FA bridging precompiled contract (`0xff0...0002`) and the first topic of the event is changed to match its ABI signature.
