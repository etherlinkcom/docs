...
octez-client -E https://mainnet.ecadinfra.com \
  run view get_voting_state on contract KT1VGyd2cRSHoDnxDnSuqGJD3mL8DzcVqX98
```
...
```bash
octez-client transfer 0 from my_wallet to KT1VGyd2cRSHoDnxDnSuqGJD3mL8DzcVqX98 \
...
```bash
octez-client call KT1VGyd2cRSHoDnxDnSuqGJD3mL8DzcVqX98 from my_wallet \
  --entrypoint new_proposal \
  --arg 'Pair "<PUBLIC_KEY>" <L2_ADDRESS>'
```
...
```bash
octez-client call KT1VGyd2cRSHoDnxDnSuqGJD3mL8DzcVqX98 from tz1RLPEeMxbJYQBFbXYw8WHdXjeUjnG5ZXNq \
  --entrypoint "vote" --arg '"yea"'
```
