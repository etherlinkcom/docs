...
octez-client call KT1Ut6kfrTV9tK967tDYgQPMvy9t578iN7iH from <MY_BAKER> \
  --entrypoint propose_voting_key \
  --arg '(Pair "<MY_VOTING_KEY>" False (Some { "KT19oUVQPnVLuUBYXrBVd46WJnNAMpqkKSwo" }))' --burn-cap 0.1
```
...
```bash
octez-client run view is_voting_key_of on contract KT1Ut6kfrTV9tK967tDYgQPMvy9t578iN7iH \
  with input 'Pair "<MY_VOTING_KEY>" "<MY_BAKER>" (Some "KT1AXRU3wLc87WNhLhVGrgqDGubLACUMUgPb")'
```
...
```bash
ligo compile expression cameligo '("<MY_VOTING_KEY>" : address), ("<MY_BAKER>" : address), (Some ("KT1AXRU3wLc87WNhLhVGrgqDGubLACUMUgPb" : address): address option)'
```
...
(Pair "tz1fsVnw7VQD73kUDB8ZWc67GWvCjTEibi9A"
      ...
      (Some "KT1AXRU3wLc87WNhLhVGrgqDGubLACUMUgPb"))
```
