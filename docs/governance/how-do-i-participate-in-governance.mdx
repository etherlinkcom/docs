...
For convenience, bakers can delegate their Etherlink voting rights to a key other than their baking key, creating an Etherlink voting key.
...
1. The baking key proposes delegating its rights to a voting key by calling the `propose_voting_key` entrypoint.
...
Suppose that a baking key delegates rights for one contract to a voting key and the voting key claims them.
...
The rights for a given baking key can vote only once, even if its voting key changes.

### Delegating voting rights for all contracts to a voting key

To set up a voting key, call the `propose_voting_key` entrypoint of the voting rights delegation contract.
By default, when you delegate rights from your baking key to a voting key, that voting key receives the rights to vote for all governance contracts.
For example, the following command proposes delegating voting rights for all governance contracts from the key with the alias `my_wallet`.
...
### Delegating rights for specific contracts to a voting key
...
For example, the following command verifies that the key represented by the placeholder `<MY_VOTING_KEY>` has the rights to vote for the key represented by the placeholder `<MY_BAKER>`.
It returns True if the `<MY_BAKER>` key has delegated rights to the `<MY_VOTING_KEY>` key for all contracts that use this contract for delegation or False if it has not.
...
To remove rights from your voting key and return them to the baking key, call the `propose_voting_key` entrypoint with the parameter `(Pair <MY_VOTING_KEY> False None)`, as in this example:
...
If you have delegated rights to other voting keys, you must run the same command for those keys.
