...
Separate contracts manage the kernel updates, security updates, and the sequencer operator, so to interact with them, bakers send transactions to those contracts, such as with the Octez client.
