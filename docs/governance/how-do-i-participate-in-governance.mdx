...
Delegating voting rights is a two-step process; delegation is not active until both of these steps are complete:
...
However, voting keys claim rights with the `claim_voting_rights` entrypoint only once.
Suppose that a baking key delegates rights for one contract to a voting key and the voting key claims it.
Then, the baking key delegates rights for a second contract to the same voting key.
...
In fact, voting keys should not call the `claim_voting_rights` entrypoint more than once because doing so ends delegation to them.

As when bakers vote with their key directly, voting keys can vote only once.
The rights for a given baking key can vote only once, even if the delegation changes.

### Delegating voting rights for all contracts
...
octez-client call KT1Ut6kfrTV9tK967tDYgQPMvy9t578iN7iH from my_wallet \
  ...
  --arg "(Pair \"<MY_DELEGATE>\" True None)" --burn-cap 0.1
```

Then, to claim the delegated voting rights, call the `claim_voting_rights` entrypoint from the voting key and pass the baking key address.
...
```bash
octez-client call KT1Ut6kfrTV9tK967tDYgQPMvy9t578iN7iH from <MY_DELEGATE> \
...
Claiming delegated rights a second time with the `claim_voting_rights` entrypoint with the voting key ends the delegation.

:::

### Delegating rights for specific contracts
...
```bash
octez-client call KT1Ut6kfrTV9tK967tDYgQPMvy9t578iN7iH <MY_DELEGATE> \
  --entrypoint claim_voting_rights \
  --arg "\"<MY_BAKER>\""
```
...
octez-client call KT1Ut6kfrTV9tK967tDYgQPMvy9t578iN7iH from <MY_BAKER> \
  --entrypoint propose_voting_key \
  --arg "(Pair \"<MY_DELEGATE>\" False (Some { \"KT1QDgF5pBkXEizj5RnmagEyxLxMTwVRpmYk\" }))" --burn-cap 0.1
```

:::tip

The source code for the delegation contract is in the file [`delegated_governance.mligo`](https://gitlab.com/tezos/tezos/-/blob/master/etherlink/tezos_contracts/governance/contracts/delegated_governance.mligo?ref_type=heads) in the Octez repository.
You can install LIGO and use its `compile parameter` command and the source code to compile parameters to call it.
For example, this command compiles the parameter to propose delegation rights for two contracts:

```bash
ligo compile parameter -m DelegateContract delegated_governance.mligo 'Propose_voting_key ("<MY_DELEGATE>", true, Some (Set.literal [("KT1VZVNCNnhUp7s15d9RsdycP7C1iwYhAQ8r" : address); ("KT1WckZ2uiLfHCfQyNp1mtqeRcC1X6Jg2Qzf" : address)]))'
```

You can escape the double quotes in the result and use it as the parameter to pass to the delegation contract.
...
(Right
   (Pair "<MY_DELEGATE>"
         ...
         (Some { "KT1VZVNCNnhUp7s15d9RsdycP7C1iwYhAQ8r" ;
                 "KT1WckZ2uiLfHCfQyNp1mtqeRcC1X6Jg2Qzf" })))
```

However, the `ligo compile parameter` command returns the parameter to call the entire contract, not the entrypoint as with the examples above that use the `--entrypoint` argument.
Therefore, to use this compiled parameter, pass it as the `--arg` argument without without the `--entrypoint` argument, as in this example:

```bash
octez-client transfer 0 from <MY_BAKER> to KT1Ut6kfrTV9tK967tDYgQPMvy9t578iN7iH \
  --arg "(Right (Pair \"<MY_DELEGATE>\" True (Some { \"KT1VZVNCNnhUp7s15d9RsdycP7C1iwYhAQ8r\" ; \"KT1WckZ2uiLfHCfQyNp1mtqeRcC1X6Jg2Qzf\" })))" --burn-cap 0.1
```
...
For example, the following command verifies that the key represented by the variable `<MY_DELEGATE>` has the rights to vote for the key represented by the variable `<MY_BAKER>`.
It returns True if the `<MY_BAKER>` key has delegated rights to the `<MY_DELEGATE>` key for all contracts that use this contract for delegation or False if it has not.

```bash
octez-client run view is_voting_key_of on contract KT1Ut6kfrTV9tK967tDYgQPMvy9t578iN7iH \
  with input "Pair \"<MY_DELEGATE>\" \"<MY_BAKER>\" None"
```

To check delegation rights for a specific contract, pass the address as an option.
For example, this command verifies delegation for the slow kernel governance contract:

```bash
octez-client run view is_voting_key_of on contract KT1Ut6kfrTV9tK967tDYgQPMvy9t578iN7iH \
  with input "Pair \"<MY_DELEGATE>\" \"<MY_BAKER>\" (Some \"KT1VZVNCNnhUp7s15d9RsdycP7C1iwYhAQ8r\")"
```

:::tip

Similar to how you can compile parameters for the entrypoints as described in [Delegating rights for specific contracts](#delegating-rights-for-specific-contracts), you can use the `ligo compile expression` command to compile the parameter for the view.
...
```bash
ligo compile expression cameligo "(\"<MY_DELEGATE>\" : address), (\"<MY_BAKER>\" : address), (Some (\"KT1VZVNCNnhUp7s15d9RsdycP7C1iwYhAQ8r\" : address): address option)"
```

You can escape the double quotes in the result and use it as the parameter to pass to the view.
Here is the result of the command:
...
### Ending delegated rights

To remove the delegation from your voting key, call the `propose_voting_key` entrypoint with the parameter `(Pair <MY_DELEGATE> False None)`, as in this example:

```bash
octez-client call KT1Ut6kfrTV9tK967tDYgQPMvy9t578iN7iH from my_wallet \
  --entrypoint propose_voting_key \
  --arg "(Pair \"<MY_DELEGATE>\" False None)" --burn-cap 0.1
```

This command revokes the delegated rights for only the specified voting key.
If you have delegated rights to other keys, you must run the same command for those keys.
...
After a proposal wins a vote, any account with voting rights can trigger the kernel upgrade by calling the appropriate governance contract's `trigger_kernel_upgrade` entrypoint:
...
After a proposed account wins a vote, any account with voting rights can trigger the change and enable that account to run the sequencer by calling the governance contract's `trigger_committee_upgrade` entrypoint:
