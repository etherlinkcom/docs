...
However, delegates claim rights with the `claim_voting_rights` entrypoint only once.
...
As when bakers vote with their key directly, delegated keys can vote only once.
...
Claiming delegated rights a second time with the `claim_voting_rights` entrypoint with the delegated key ends the delegation.
...
```bash
ctez-client call KT1Ut6kfrTV9tK967tDYgQPMvy9t578iN7iH <MY_DELEGATE> \
  --entrypoint claim_voting_rights \
  --arg "\"<MY_BAKER>\""
```
