...
This command compiles an expression of this CameLIGO type to Michelson to propose delegation rights for two contracts:

```bash
ligo compile expression cameligo "(\"<MY_DELEGATE>\" : address), True, (Some (Set.literal [(\"KT1WckZ2uiLfHCfQyNp1mtqeRcC1X6Jg2Qzf\" : address); (\"KT1VZVNCNnhUp7s15d9RsdycP7C1iwYhAQ8r\" : address)]) : (address set) option)"
```

You can escape the double quotes in the result and use it as the parameter to pass to the `propose_voting_key` entrypoint.
...
octez-client call KT1Ut6kfrTV9tK967tDYgQPMvy9t578iN7iH from <MY_BAKER> \
  --entrypoint propose_voting_key \
  --arg "(Pair \"<MY_DELEGATE>\" True (Some { \"KT1VZVNCNnhUp7s15d9RsdycP7C1iwYhAQ8r\" ; \"KT1WckZ2uiLfHCfQyNp1mtqeRcC1X6Jg2Qzf\" }))"
```
:::
