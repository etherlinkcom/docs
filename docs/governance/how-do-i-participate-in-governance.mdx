...
It uses the placeholder `<MY_DELEGATE>` in place of the address of the account to delegate to:
...
It uses the placeholder `<MY_BAKER>` in place of the address of the baking key address:
...
For example, the following command verifies that the key represented by the placeholder `<MY_DELEGATE>` has the rights to vote for the key represented by the placeholder `<MY_BAKER>`.
