...
One commit that is known to work well is [`c5ed8f9`](https://gitlab.com/tezos/tezos/-/commit/c5ed8f9d5737cec03b29e5a7c3bf2169cc239ea8)
from the `master` branch of the [Octez
...
For more information, see the Octez documentation: https://tezos.gitlab.io/index.html

For simplicity, these steps show how to run the node in observer mode.

1. Build the source code for the Smart Rollup node or get a binary build of `octez-smart-rollup-node`.
...
1. Activate the `unsafe-pvm-patches` setting, which is required for Etherlink, by adding it to the `config.json` file, as in this example:
   ...
   This example also sets the preimages endpoint, which allows the Smart Rollup node to retrieve the initial kernel of Etherlink from an arbitrary source without any safety risks.
...
1. Start the Smart Rollup node in observer mode by running this command and using the RPC endpoint of a layer 1 node that is running in archive mode:
   ...
   For development use, you must use a layer 1 node that is running in archive mode and provides access to its RPC endpoints.
...
### Building Etherlink's installer kernel

The kernel of Etherlink is a Rust program compiled in WASM implementing the semantics of Etherlink blocks and transactions.
It is used by Etherlink nodes and Smart Rollup nodes to validate the blocks created by Etherlink's sequencer.

Because Octez does not yet provide binaries for Etherlink nodes, you must build the Etherlink installer kernel yourself.
This installer kernel is a compressed version of the kernel that provides only enough information to install the original kernel.
...
However, if you want to start an Etherlink node from its genesis block without trusting these files, you must build the installer kernel yourself.
...
The content of `installer.hex` is the following code.
The Etherlink node does not verify this code when you use it to start the node.
Therefore, you should build it yourself.
...
- The `installer.hex` file that you can get by following the instructions in [Building Etherlink's installer kernel](#building-etherlinks-installer-kernel)
