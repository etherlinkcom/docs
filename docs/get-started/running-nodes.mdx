...
Because public Smart Rollup nodes for Etherlink are not yet available, to run an Etherlink node, you must also run a Smart Rollup node.
...
- Octez does not yet provide a binary build of the Etherlink node as part of its binary distribution or in the `tezos/tezos` docker image, so you must build it yourself.
