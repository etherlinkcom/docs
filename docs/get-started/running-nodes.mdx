...
To run an Etherlink node, you need access to a Smart Rollup node set up for Etherlink and to an ordinary Tezos layer 1 node.
...
- Public Smart Rollup nodes for Etherlink are not yet available, so you must run your own as described in [Running a Smart Rollup node for Etherlink](#running-a-smart-rollup-node-for-etherlink).
You can start this node with a snapshot of the Etherlink state or from Etherlink genesis.
Setting up the Smart Rollup node from Etherlink genesis requires a recent build of the Octez suite.
- For the Etherlink node itself, Octez does not yet provide a binary build of the Etherlink node as part of its binary distribution or in the `tezos/tezos` docker image, so you must build it yourself.
However, you can get the binary from the `tezos/tezos-bare` image if you use a specific tag.
The tag `master_b9f6c913_20240424112144` includes the Etherlink node and matches other examples on this page.
...
1. Get a built version of the Smart Rollup node binary, named `octez-smart-rollup-node`, in one of these ways:
   ...
   One commit that is known to work well is [`b9f6c91`](https://gitlab.com/tezos/tezos/-/commit/b9f6c9138719220db83086f0548e49c5c4c8421f) from the `master` branch of the [Octez repository](https://gitlab.com/tezos/tezos).
