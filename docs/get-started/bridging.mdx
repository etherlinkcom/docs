...
1. A user sends tez to the Etherlink Smart Rollup.
The request includes their layer 1 address.
1. Etherlink nodes forward the transaction to the sequencer.
1. The sequencer burns the tez and creates a ticket that represents the tez.
1. The sequencer puts the ticket in a call to the exchanger contract's `burn` entrypoint and puts this transaction in the Smart Rollup outbox on layer 1.
