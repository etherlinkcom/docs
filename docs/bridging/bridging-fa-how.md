...
1. The null address precompile sends the information about the deposit to the FA bridge precompiled contract (`0xff0...0002`), which then emits a `QueuedDeposit` event which contains the `depositId` information needed to complete the transfer.

1. Any user can call the FA bridge precompiled contract's `claim` function, which causes the contract to call the ERC-20 proxy contract.
...
1. The user calls the FA bridge precompiled contract on Etherlink and includes this information:
