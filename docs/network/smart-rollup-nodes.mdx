---
title: Running a Etherlink Smart Rollup node # tx
dependencies:
  octez: 0
---
...
Running a Etherlink{/* TX */} Smart Rollup node on Etherlink{/* TX */} Mainnet requires a computer with 500GB of disk space and at least 16GB RAM.
...
1. To speed up the setup process by loading a snapshot, follow these steps:
   ...
   <TabItem value="shadownet" label={"Etherlink Shadownet Testnet" /* TX */}>
      ...
      ```bash
      octez-smart-rollup-node --endpoint https://rpc.tzkt.io/shadownet \
        snapshot import <SNAPSHOT_URL_OR_FILE> \
        --data-dir <SR_DATA_DIR>
      ```
...
1. Start the Smart Rollup node in observer mode with the `run` command.
   ...
   ```bash
   octez-smart-rollup-node --endpoint https://rpc.tzkt.io/shadownet run \
     --data-dir <SR_DATA_DIR>
   ```
...
Follow these steps to stop a Etherlink{/* TX */} Smart Rollup node:
...
- `<SMART_ROLLUP_ADDRESS>`: The address of the Etherlink{/* TX */} Smart Rollup
