...
If the Tezos [Data Availability Layer](https://docs.tezos.com/architecture/data-availability-layer) (DAL) is active, the sequencer publishes the block via the DAL; otherwise, it publishes the block directly to Tezos layer 1 via the Smart Rollup outbox.
...
- It publishes them to Tezos layer 1 via the DAL or via the Smart Rollup outbox, which finalizes the transactions.
