...
1. Run the `octez-evm-node run observer` command to start the node as an observer:
    ...
    <Tabs groupId="chain">
      ...
      At this time, only archive snapshots are available for Etherlink Shadownet Testnet.
      If you want to run the node in a different mode, start it in archive mode and then switch modes as described in [Switching history modes](#switching-history-modes).
