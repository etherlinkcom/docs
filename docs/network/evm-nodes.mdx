...
- If you want to verify the blocks that come from the sequencer, run a Etherlink{/* TX */} Smart Rollup node as described in [Running a Etherlink{/* TX */} Smart Rollup node](/network/smart-rollup-nodes).
...
1. Download the latest release of the `octez-evm-node` binary from https://gitlab.com/tezos/tezos/-/releases.
   ...
   :::note
   Version 0.64 or later of the `octez-evm-node` binary is required to suppport the Etherlink{/* TX */} kernel 7.0.
   :::

1. Run the `octez-evm-node run observer` command to start the node as an observer:

    <Tabs groupId="chain">
      <TabItem value="mainnet" label={"Etherlink Mainnet" /* TX */}>
      ...
      <TabItem value="shadownet" label={"Etherlink Shadownet Testnet" /* TX */}>
...
- The network to use, which you specify in the `--network` argument or the `EVM_NODE_NETWORK` environment variable.
    ...
    In this way, setting the `--network` argument implies these arguments:
   ...
   <TabItem value="shadownet" label="Etherlink Shadownet Testnet">
      ...
      ```bash
      --preimages-endpoint https://snapshots.tzinit.org/etherlink-shadownet/wasm_2_0_0 \
      --evm-node-endpoint https://relay.shadownet.etherlink.com
      ```
...
1. Create the configuration file by setting the parameters in the environment variables or passing the arguments to the `octez-evm-node init config` command.
   ...
   If you are not running a Etherlink{/* TX */} Smart Rollup node and are trusting incoming blocks, use the `--dont-track-rollup-node` flag, as in this `mainnet` example:
   ...
   If you set the `EVM_NODE_DATA_DIR` and `EVM_NODE_NETWORK` environment variables, you can omit the `--network` and `--data-dir` arguments, as in this `mainnet` example:
   ...
   If you want to rely on a Smart Rollup node to check the correctness of blocks coming from the sequencer, pass the address of the Smart Rollup node to the `--rollup-node-endpoint` argument, as in this `mainnet` example:
...
The EVM node generates a configuration file and puts it in the location that you specified in the command.
...
<Tabs groupId="chain">
  ...
  ```bash
  export EVM_NODE_DATA_DIR="<EVM_DATA_DIR>"
  export EVM_NODE_NETWORK=shadownet
  octez-evm-node run observer \
    --history rolling:1 \
    --dont-track-rollup-node \
    --init-from-snapshot
...
The following `mainnet` examples use `<SR_OBSERVER_DATA_DIR>` as the location of this temporary directory and `<SNAPSHOT_URL_OR_FILE>` as the snapshot URL or file name.
...
Then when you restart the node, run `octez-evm-node run observer --network <NETWORK> --history rolling:<DAYS> --data-dir <EVM_DATA_DIR>`.
...
If you switch the node from archive mode to full mode, it reduces the size of the data directory the next time that the node runs internal garbage collection, within 24 hours.
Switching between other modes does not immediately reduce the size of the data directory.
...
For example, this command gets the number of the most recent EVM block in hexadecimal:
