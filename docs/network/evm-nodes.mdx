...
1. Download the latest release of the `octez-evm-node` binary from https://gitlab.com/tezos/tezos/-/releases.
   ...
   :::note
   As of August 1st 2025, Etherlink Mainnet and Ghostnet or Shadownet Testnets require at least version 0.35 of the `octez-evm-node` binary.
   :::
...
1. Run the `octez-evm-node run observer` command to start the node as an observer:
    ...
    <Tabs groupId="chain">
      ...
      Use this command for Etherlink Shadownet Testnet (coming soon):
      ...
      :::important
      Until the EVM Node snapshot service for Shadownet is available, please use this temporary command to bootstrap the node from the genesis block:

      `octez-evm-node run observer --network braeburn --dont-track-rollup-node`
...
<Tabs groupId="chain">
  ...
  <TabItem value="shadownet" label="Etherlink Shadownet Testnet (coming soon)">
