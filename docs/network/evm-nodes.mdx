...
1. Download the latest release of the `octez-evm-node` binary from https://gitlab.com/tezos/tezos/-/releases.
   ...
   :::note
   As of version 6.1, Etherlink requires at least version 0.50 of the `octez-evm-node` binary.
   :::
...
1. Run the `octez-evm-node run observer` command to start the node as an observer:
    ...
    <Tabs groupId="chain">
      ...
      <TabItem value="ghostnet" label="Etherlink Ghostnet Testnet">
      ...
      :::important
      Etherlink Ghostnet Testnet will sunset on **15 January 2026**, and will then no longer be available. Please [migrate](/network/migrating-testnet) to Etherlink Shadownet Testnet.
      :::
...
- The network to use, which you specify in the `--network` argument or the `EVM_NODE_NETWORK` environment variable.
    ...
    - `--network testnet` for Etherlink Ghostnet
   ...
   <TabItem value="ghostnet" label="Etherlink Ghostnet Testnet">
    ...
    ```bash
    --preimages-endpoint https://snapshots.tzinit.org/etherlink-testnet/wasm_2_0_0 \
    --evm-node-endpoint https://relay.testnet.etherlink.com
    ```
...
1. Get the URL of the Etherlink Smart Rollup node snapshot for the appropriate Etherlink network or download the file manually:
   ...
   - Etherlink Ghostnet Testnet: https://snapshots.tzinit.org/etherlink-ghostnet
