...
The best way to run a node in operator mode is to start with a node in observer mode, verify that it works and convert it to operator mode.
...
You can start the Smart Rollup node in observer mode to bootstrap it and make sure that it works before you change to operator mode.
...
## Converting the Smart Rollup node to operator mode

Converting the observer node to operator mode requires these prerequisites:
...
Follow these steps to convert a Smart Rollup node from observer mode to operator mode:
...
1. Initialize the local context of the node, which is where it stores local data:
   ...
   1. Initialize the local context by running this command and using the address of the Etherlink Smart Rollup, which you can get from the [Network information](/get-started/network-information) page:
      ...
      octez-smart-rollup-node init observer config for sr18wx6ezkeRjt1SZSeZ2UQzQN3Uc3YLMLqg \
        with operators --data-dir $SR_DATA_DIR \
        --pre-images-endpoint https://snapshots.eu.tzinit.org/etherlink-testnet/wasm_2_0_0
      ```
