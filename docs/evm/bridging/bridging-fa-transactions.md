...
## Depositing FA tokens from layer 1 to Etherlink EVM<!--TEVM-->

It takes a few transactions to bridge a token from layer 1 to Etherlink EVM<!--TEVM-->:
...
1. Call the token bridge helper's `deposit` entrypoint and pass the address of the Etherlink<!--TX--> Smart Rollup and the address of the Etherlink EVM<!--TEVM--> account to send the tokens to, as in this example:
   ...
   The token bridge helper contract sends that ticket to Etherlink EVM<!--TEVM-->.

1. When the deposit is in an Etherlink EVM<!--TEVM--> block, call the FA bridging precompiled contract contract's `claim` function to cause the ERC-20 proxy contract to mint the tokens.
   ...
   The precompiled contract sends information about the deposit to the ERC-20 proxy contract, which mints the tokens and sends them to the Etherlink EVM<!--TEVM--> account.
...
To see the tokens in your Etherlink EVM<!--TEVM--> wallet, look up the ERC-20 proxy contract in a block explorer or use its address to manually add the tokens to your wallet.
Because the Etherlink EVM<!--TEVM--> tokens are compatible with the ERC-20 standard, EVM-compatible wallets should be able to display them.
...
Initiating the withdrawal requires sending complex information about the ticket and contracts to the FA2 withdrawal precompile on Etherlink EVM<!--TEVM-->.
Running the outbox transaction requires you to know the level of the Etherlink<!--TX--> commitment that contains it in order to get its proof and commitment, and there is no easy way to get that information without using an indexer to check each level for the transaction.
