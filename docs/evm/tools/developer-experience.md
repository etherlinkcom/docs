...
[Etherlink Safe](https://safe.etherlink.com) is an Etherlink EVM<!--TEVM--> implementation of [Safe\{Wallet\}](https://github.com/safe-global/safe-wallet-web), which manages multi-signature smart contracts (multisigs).
...
Gelato Web3 Functions supports Etherlink EVM<!--TEVM-->, which lets you automate Etherlink EVM<!--TEVM--> contracts based on off-chain activity and activity on other chains.
...
Select the network to monitor (such as Etherlink<!--TX--> Testnet or Mainnet), specify the address and ABI of the contract, and the event to trigger on, as in this screencap:
