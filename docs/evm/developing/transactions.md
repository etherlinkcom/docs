...
- For a list of endpoints that Etherlink EVM<!--TEVM--> supports, see [Ethereum endpoint support](/evm/developing/endpoint-support)
...
Etherlink EVM<!--TEVM--> does not support the `eth_sendTransaction` endpoint.
...
For more information, see [Estimating fees](/evm/developing/estimating-fees).
...
Beginning with EVM node 0.52 and the version 6.0 upgrade, Etherlink<!--TX--> supports Instant Confirmations.
...
To receive Instant Confirmations with the lowest possible latency, use an EVM node<!--TXN--> as close as possible to the sequencer, which is deployed to a data center in Tokyo, Japan.
...
See [Subscribing to Instant Confirmations](/evm/developing/websockets#subscribing-to-instant-confirmations).
...
In this case, you can set up your own EVM node<!--TXN--> and send batched transactions to it.
...
Etherlink EVM<!--TEVM--> does not support listening for smart contract events with endpoints such as `eth_newFilter` and `eth_getFilterChanges`.
...
To get updates about activity on Etherlink EVM<!--TEVM--> via WebSockets, see [Subscribing to events](/evm/developing/websockets#subscribing-to-events).
...
Etherlink EVM<!--TEVM--> supports the `debug_traceTransaction` and `debug_traceBlockByNumber` RPC endpoints, which provide debugging information about transactions and blocks.
...
Etherlink EVM<!--TEVM--> supports only the `callTracer` tracer for this endpoint, and only for blocks after the Bifröst upgrade.
...
Delayed inbox transactions cost 1 tez (on Tezos, charged to the account that submits it) in addition to the normal Etherlink EVM<!--TEVM--> execution fee.
However, delayed inbox transactions do not require an inclusion fee as described in [Fee structure](/evm/developing/fees).
Therefore, you can calculate the execution fee as on any other Ethereum transaction and use that as the execution fee.
This transaction fee is deducted from the Etherlink EVM<!--TEVM--> account that signed the transaction.

The general steps for submitting a transaction to the delayed inbox are:

1. Encode and sign the Etherlink EVM<!--TEVM--> transaction as usual with a library such as ethers.js.
...
- `evm_rollup`: The address of the Etherlink<!--TX--> Smart Rollup, which you can get from [Network information](/evm/get-started/network-information)
- `transaction`: The signed transaction to run on Etherlink EVM<!--TEVM-->
