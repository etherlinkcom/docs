...
As described in [Fee structure](/evm/developing/fees), Etherlink EVM<!--TEVM--> fees include the cost of running the transaction and writing the transaction to layer 1 but not a voluntary tip.

:::tip

You can use libraries such as [ethers.js](https://docs.ethers.org/v6/) to estimate transaction fees.
For an example, see [Sending transactions](/evm/developing/transactions).

:::

Etherlink EVM<!--TEVM--> supports the standard EVM `eth_gasPrice` endpoint to provide the current gas price, as in this example:
...
This fee includes all fees described in [Fee structure](/evm/developing/fees), but you can set a higher amount for the transaction's gas consumption as a safety margin (sometimes known as setting the gas limit) to ensure that if the transaction requires more gas, it can use it.
...
The example in [Sending XTZ](/evm/developing/transactions#sending-xtz) sets a maximum gas price by setting the `maxFeePerGas` field on the transaction object.
