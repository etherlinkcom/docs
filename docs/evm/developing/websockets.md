...
The EVM node<!--TXN--> supports requests over WebSockets.
...
Another advantage of using WebSockets on Etherlink EVM<!--TEVM--> is support for the `eth_subscribe` method.
This method allows clients to subscribe to Etherlink EVM<!--TEVM--> events and receive notifications when these events happen, such as new blocks, transactions in the pool, and smart contract event logs.
...
You can use any WebSocket framework to connect to EVM nodes<!--TXN-->.
...
1. Set up an EVM node as described in [Running an EVM node<!--TXN-->](/network/evm-nodes).
...
For example, you can use the `wscat` program to send and receive messages over a WebSocket connection on the command line:
   ...
   For example, this RPC request gets the current block number:
      ...
      For other RPC methods that the EVM node supports, see [Ethereum endpoint support](/evm/developing/endpoint-support).

Now clients can initiate connections over WebSockets.
For examples with other clients that can use WebSocket connections with EVM nodes<!--TXN-->, see [Using WebSockets for instant notifications from Etherlink](https://medium.com/@etherlink/using-websockets-for-instant-notifications-from-etherlink-b70d1c79c7a1).
...
For example, if you subscribe to the `newBlockHeaders` event, the node sends a message to the client when each new Etherlink EVM<!--TEVM--> block (blueprint) is created.
...
EVM nodes<!--TXN--> provide two custom WebSocket events that you can subscribe to for notice of upcoming transactions:
...
For more information about Instant Confirmations, see [Getting Instant Confirmations](/evm/developing/transactions#getting-instant-confirmations)

## WebSocket subscriptions

You can use WebSockets to subscribe to these Etherlink EVM<!--TEVM--> events:

- `newBlockHeaders`: Provides information about each new Etherlink EVM<!--TEVM--> block, including the block number, the hash of the parent block, and a list of the transactions in that block
